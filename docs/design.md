...
The macOS and iOS simulator fixture UI smoke templates are manual-dispatch only. They read the private adapter from `APPLE_AGENT_KIT_ADAPTER_JSON`, require an exact `fixture-ui-smoke` approval input, run the private fixture command, validate the receipt, and upload adapter-allowlisted evidence only after the receipt gate succeeds. The iOS simulator lane uses `platforms.ios.simulatorDestination` and still forbids physical-device actions.
