...
The device-free self-hosted Mac runner-health, macOS eligibility, and macOS fixture UI smoke templates render from `ci.macosRunnerLabels`. The iOS eligibility and iOS simulator fixture templates default to a hosted macOS runner from `ci.macosRunner`. Keep these separate from physical iOS runner labels so an eligibility check does not imply install, WDA, screenshots, app launch, UI smoke, simulator boot, or physical-device smoke.

The macOS and iOS simulator fixture UI smoke templates are manual-dispatch only. They read the private adapter from `APPLE_AGENT_KIT_ADAPTER_JSON`, require an exact `fixture-ui-smoke` approval input, remove any pre-existing receipt at the configured path, run the private fixture command, validate the receipt even after a failed smoke command, and upload adapter-allowlisted evidence only when receipt validation succeeds. This keeps failures diagnosable when the private command emits a valid failed receipt during the current run without bypassing the privacy gate. The iOS simulator lane uses `platforms.ios.simulatorDestination` and still forbids physical-device actions.
