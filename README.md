...
For the first real GUI proof after eligibility, use the `macos-fixture-ui-smoke` skill with a private `automation` adapter block. The public kit provides the generic receipt shape in `templates/fixture-ui-smoke.receipt.example.json` and a copyable command scaffold in `templates/fixture-ui-smoke-command.example.sh`; private repos supply the fixture app, smoke command, log subsystem, and evidence allowlist.

See [Adopting the macOS CI Eligibility Lane](docs/adopting-macos-ci-eligibility.md), [Adopting macOS Fixture UI Smoke](docs/adopting-macos-fixture-ui-smoke.md), [Adopting the iOS CI Eligibility Lane](docs/adopting-ios-ci-eligibility.md), [Self-Hosted Runner Scope and Onboarding](docs/self-hosted-runner-scope-onboarding.md), the [Dedicated Mac Runner-Health Checklist](docs/dedicated-mac-runner-health-checklist.md), and the [Private Adapter Rollout Checklist](docs/private-adapter-rollout-checklist.md) for a repeatable public/private adoption path. Public proof records are indexed in [Proof Records](docs/proof-records/index.md).
